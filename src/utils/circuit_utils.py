...
def save_bench(n_var, cnf, bench_file):
    f = open(bench_file, 'w')
    for pi_idx in range(1, n_var+1, 1):
        f.write('INPUT(N{:})\n'.format(pi_idx))
    f.write('OUTPUT(PO)\n')
    f.write('\n')

    # NOT
    has_not = [False] * (n_var + 1)
    for clause in cnf:
        for var in clause:
            if var < 0:
                has_not[int(abs(var))] = True
    for pi_idx in range(1, n_var+1, 1):
        if has_not[pi_idx]:
            f.write('N{:}_NOT = NOT(N{:})\n'.format(pi_idx, pi_idx))

    # Clause
    for clause_idx, clause in enumerate(cnf):
        if len(clause) == 1:
            if clause[0] > 0:
                new_line = 'CLAUSE_{:} = BUFF(N{:})\n'.format(clause_idx, clause[0])
            else:
                new_line = 'CLAUSE_{:} = BUFF(N{:}_NOT)\n'.format(clause_idx, int(abs(clause[0])))
        else:
            new_line = 'CLAUSE_{:} = OR('.format(clause_idx)
            for var_idx, var in enumerate(clause):
                if var > 0:
                    new_line += 'N{:}'.format(var)
                else:
                    new_line += 'N{:}_NOT'.format(int(abs(var)))
    ...
    # PO
    if len(cnf) == 1:
        new_line = 'PO = BUFF(CLAUSE_0)\n'
    else:
        new_line = 'PO = AND('
        for clause_idx in range(len(cnf)):
            new_line += 'CLAUSE_{:}'.format(clause_idx)
...
def parse_x_data(x_data, edge_index_data):
    fanout_list = []
    fanin_list = []
    bfs_q = []
...
def feature_generation(data, gate_to_index):
    ...
    return parse_x_data(x_data, edge_index_data)
...
def mask_simulator(x_data, PI_indexes, level_list, fanin_list, num_patterns):
    '''
       Logic simulator
       ...
       Parameters:
    ...
    # print('[INFO] Begin simulation')
    while pattern_count < no_of_patterns and success_count < 10000:
        if len(PI_indexes) <= 16:
            if pattern_count == int(pow(2, len(PI_indexes))):
                break
            input_vector = dec2list(pattern_count, len(PI_indexes))
        else:
            input_vector = random_pattern_generator(len(PI_indexes))
        failed = False
        ...
        for level in range(1, len(level_list), 1):
            ...
            for node_idx in level_list[level]:
                ...
                if len(source_signals) > 0:
                    gate_type = x_data[node_idx][1]
                    tmp_y = logic(gate_type, source_signals)
                    if x_data[node_idx][-1] == -1:
                        y[node_idx] = tmp_y
                    elif x_data[node_idx][-1] != tmp_y:
                        failed = True
                        break
                    else:
                        y[node_idx] = tmp_y
...
def search_solution(x_data, PI_indexes, level_list, fanin_list, assignment):
    y = [0] * len(x_data)
    
    input_vector = assignment
    failed = False
