...
class V2CGNN(nn.Module):
    def __init__(self, args):
        ...
        self.buff_gate = MLP(args.pos_size, args.dim_mlp, args.dim_hidden, 
                           num_layer=args.num_fc, norm_layer=None, act_layer=args.activation_layer, sigmoid=False, tanh=False)
        self.not_gate = MLP(args.pos_size, args.dim_mlp, args.dim_hidden, 
                           num_layer=args.num_fc, norm_layer=None, act_layer=args.activation_layer, sigmoid=False, tanh=False)
    ...
    def _gru_forward(self, G, h_init):
        ...
        var_pos = torch.randn(len(var_node), self.args.pos_size)
        var_state = self.buff_gate(var_pos)
        negvar_state = self.not_gate(var_pos)
