...
class DeepSAT(nn.Module):
    ...
    def forward(self, G):
        ...
        if self.mask:
            if self.args.hs_can_train:
                h_true  =  nn.Parameter(torch.zeros_like((h_init)).to(self.device)) 
                h_false =  nn.Parameter(torch.zeros_like((h_init)).to(self.device))
            else:
        ...
        if 'lstm' in self.args.update_function:
            ...
            if self.args.nolevel:
                preds = self._gru_forward_nolevel(G, h_init, num_layers_f, num_layers_b, h_true, h_false)
            elif self.args.nodag:
                preds = self._gru_forward_nodag(G, h_init, num_layers_f, num_layers_b, h_true, h_false)
            else:
    ...
    def _gru_forward_nodag(self, G, h_init, num_layers_f, num_layers_b, h_true=None, h_false=None):
        x, edge_index = G.x, G.edge_index
        edge_attr = None
        ...
        for _ in range(self.num_rounds):
            for l_idx in range(1, num_layers_f):
                ...
                fanin_edge_index, fanin_edge_attr = subgraph(l_node, edge_index, edge_attr, dim=1)
                fanout_edge_index, fanout_edge_attr = subgraph(l_node, edge_index, edge_attr, dim=0)

                cat_edge_index = torch.cat([fanin_edge_index, fanout_edge_index], dim=1)

                msg = self.aggr_forward(node_state.squeeze(0), cat_edge_index, fanin_edge_attr)
    ...
    def _gru_forward_nolevel(self, G, h_init, num_layers_f, num_layers_b, h_true=None, h_false=None):
        x, edge_index = G.x, G.edge_index
        edge_attr = None

        node_state = h_init  # (h_0). here we initialize h_0. TODO: option of not initializing the hidden state of GRU.

        layer_mask = G.forward_level
        l_node = G.forward_index[layer_mask]
        l_state = torch.index_select(node_state, dim=1, index=l_node)
