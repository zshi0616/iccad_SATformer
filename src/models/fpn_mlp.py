...
class MLP_Windows(nn.Module):
    def __init__(self, args):
        ...
        self.mlp = MLP(args.dim_hidden, args.tf_emb_size, args.dim_hidden, num_layer=3,
                norm_layer=None, act_layer=args.activation_layer, sigmoid=False, tanh=False)
    ...
    def forward(self, x : Tensor) -> Tensor:
        ...
        x = self.mlp(windows_x)
        x = rearrange(x, 'n l d -> (n l) d')
...
class EncoderBlock_Windows(nn.Sequential):
    def __init__(self, args):
        super().__init__(
...
class NodeMLP_Hierarchical(nn.Sequential):
    def __init__(self, args):
        super().__init__(
        ...
        tf_fp_layers = [EncoderBlock_Windows(args).to(self.args.device) for _ in range(args.TF_depth)]
