...
class CnfgenDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        while len(sat_cnfs) < self.args.n_pairs or len(unsat_cnfs) < self.args.n_pairs: 
            ...
            if is_sat and len(sat_cnfs) < self.args.n_pairs:
                sat_cnfs.append(iclauses)
                sat_vars.append(n_var)
            if not is_sat and len(unsat_cnfs) < self.args.n_pairs:
                unsat_cnfs.append(iclauses)
                unsat_vars.append(n_var)

            print(len(sat_cnfs), len(unsat_cnfs))

        for problem_idx in range(self.args.n_pairs):
            sat_n_vars = sat_vars[problem_idx]
            iclauses_sat = sat_cnfs[problem_idx]
            unsat_n_vars = unsat_vars[problem_idx]
            iclauses_unsat = unsat_cnfs[problem_idx]

            if self.args.community:
                com, q = community(sat_n_vars, iclauses_sat)
                new_iclauses_sat = reorder_cnf(iclauses_sat, com)
                graph_sat = cnf_parse_pyg(self.args, new_iclauses_sat, 1, sat_n_vars, len(new_iclauses_sat))
                data_list.append(graph_sat)
                print('[INFO] # Var: {:}, C/V: {:.2f}, Modularity: {:.4f}'.format(sat_n_vars, len(new_iclauses_sat)/sat_n_vars, q))

                com, q = community(unsat_n_vars, iclauses_unsat)
                new_iclauses_unsat = reorder_cnf(iclauses_unsat, com)
                graph_unsat = cnf_parse_pyg(self.args, new_iclauses_unsat, 0, unsat_n_vars, len(new_iclauses_unsat))
                data_list.append(graph_unsat)
                print('[INFO] # Var: {:}, C/V: {:.2f}, Modularity: {:.4f}'.format(unsat_n_vars, len(new_iclauses_unsat)/unsat_n_vars, q))
                ...
                for aug_idx in range(self.args.random_augment):
                    ...
                    graph_sat = cnf_parse_pyg(self.args, iclauses_sat, 1, sat_n_vars, len(iclauses_sat))
                    graph_unsat = cnf_parse_pyg(self.args, iclauses_unsat, 0, unsat_n_vars, len(iclauses_unsat))

                    data_list.append(graph_sat)
                    data_list.append(graph_unsat)

        # while len(sat_cnfs) + len(unsat_cnfs) < self.args.n_pairs: 
        #     n = random.randint(self.args.cnfgen_n[0], self.args.cnfgen_n[1])
        #     k = random.randint(self.args.cnfgen_k[0], self.args.cnfgen_k[1])
        #     p = self.args.cnfgen_p 
        #     n_var, iclauses, is_sat = cnfgen_utils.create_problem(self.args, n, p, k)
        ...
        # problem_cnfs = sat_cnfs + unsat_cnfs
        # problem_vars = sat_vars + unsat_vars

        # for problem_idx in range(self.args.n_pairs):
        #     n_vars = problem_vars[problem_idx]
        #     iclauses = problem_cnfs[problem_idx]
        #     if problem_idx < len(sat_cnfs): 
        #         sat = 1
        #     else:
        #         sat = 0
        #     graph = cnf_parse_pyg(self.args, iclauses, sat, n_vars, len(iclauses))
